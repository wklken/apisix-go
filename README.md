...
- [x] [proxy-cache](https://apisix.apache.org/zh/docs/apisix/plugins/proxy-cache/) 52%
  - support in-memory response caching with `cache_key`, `cache_method`, `cache_http_status`, `cache_ttl`, `cache_bypass`, `no_cache`, `hide_cache_headers`, `consumer_isolation`, and `Apisix-Cache-Status`
  - not support disk cache zones, `Vary`, `Cache-Control` request/response semantics, public purge endpoint, or stale serving
