...
- [x] [limit-count](https://apisix.apache.org/zh/docs/apisix/plugins/limit-count/) 86%
  ...
  - not support `redis-cluster`
- [x] [graphql-limit-count](https://apisix.apache.org/docs/apisix/plugins/graphql-limit-count/) 82%
  - support official plugin name, priority, schema, POST `application/json` and `application/graphql` requests bounded by `graphql.max_size`, GraphQL selection-depth cost counting, local, standalone Redis, and Redis Cluster fixed-window quotas, official Redis/TLS/pool config fields, `allow_degradation`, `X-RateLimit-*` headers, `rejected_code`, `rejected_msg`, `key`, `key_type`, and fragment/inline-fragment/chained-fragment depth expansion
  - not support limit-count multi-rule/string-expression configuration, full GraphQL spec parsing parity, or exact `resty.limit.count` behavior
