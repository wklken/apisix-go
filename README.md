...
- [x] [proxy-cache](https://apisix.apache.org/zh/docs/apisix/plugins/proxy-cache/) 45%
  - support in-memory response caching with `cache_key`, `cache_method`, `cache_http_status`, `cache_ttl`, `cache_bypass`, `no_cache`, `hide_cache_headers`, and `Apisix-Cache-Status`
  - not support disk cache zones, `Vary`, `Cache-Control` request/response semantics, consumer isolation override, or stale serving
