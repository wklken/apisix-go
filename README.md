...
- [x] [cors](https://apisix.apache.org/zh/docs/apisix/plugins/cors/) 55%
  - support `allow_origins`, `allow_origins_by_regex`, methods/headers/exposed headers, `max_age`, and `allow_credential`
  - not support `allow_origins_by_metadata`, timing allow origins, or exact APISIX `*` / `**` wildcard semantics for every option
