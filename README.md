...
- [x] [batch-requests](https://apisix.apache.org/zh/docs/apisix/plugins/batch-requests/) 60%
  - support `/apisix/batch-requests`, global and per-item query/header merging, default methods, request body forwarding, response aggregation, and `plugin_attr.batch-requests.uri`
  - not support true HTTP pipelining, plugin metadata `max_body_size` / `max_pipeline_items` overrides, real-ip header injection from NGINX config, or `ssl_verify`
...
- [x] [limit-conn](https://apisix.apache.org/zh/docs/apisix/plugins/limit-conn/) 60%
  - only support `policy = "local"`
  - not support `rules`
  - not support `redis` or `redis-cluster`
...
- [x] [proxy-cache](https://apisix.apache.org/zh/docs/apisix/plugins/proxy-cache/) 58%
  - support in-memory response caching with `cache_key`, `cache_method`, `cache_http_status`, `cache_ttl`, `cache_bypass`, `no_cache`, `hide_cache_headers`, `consumer_isolation`, `cache_control` request bypass for `no-cache` / `no-store`, upstream `private` / `no-store` / `no-cache` non-storage, and `Apisix-Cache-Status`
  - not support disk cache zones, `Vary`, full `Cache-Control` TTL / `Expires` / `only-if-cached` / stale semantics, public purge endpoint, or stale serving
...
- [x] [loggly](https://apisix.apache.org/zh/docs/apisix/plugins/loggly/) 45%
  ...
  - not support HTTP/S bulk endpoint, batch processor options, or request/response body capture
...
- [x] [loki-logger](https://apisix.apache.org/zh/docs/apisix/plugins/loki-logger/) 50%
  - support `endpoint_addrs`, `endpoint_uri`, `tenant_id`, custom headers, `log_labels`, `ssl_verify`, `timeout`, and `log_format`
  - not support random endpoint selection, APISIX batch processor behavior, `max_pending_entries`, or request/response body capture
- [x] [lago](https://apisix.apache.org/docs/apisix/plugins/lago/) 55%
  - support official plugin name, priority, schema, Lago batch event payload shape, Bearer token delivery to `endpoint_uri`, request/response variable templates for transaction ID, subscription ID, and event properties, `ssl_verify`, timeout, and keepalive config defaults
  - not support APISIX batch processor buffering/retry semantics, random endpoint selection, full APISIX/NGINX variable coverage, request start-time fidelity, or request/response body capture
