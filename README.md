...
- [x] [limit-conn](https://apisix.apache.org/zh/docs/apisix/plugins/limit-conn/) 87%
  ...
  - not support `redis-cluster`
- [x] [limit-count](https://apisix.apache.org/zh/docs/apisix/plugins/limit-count/) 92%
  - support local, Redis, and Redis Cluster fixed-window quotas, APISIX root-level Redis/Redis Cluster fields and legacy nested configs, TLS/pool settings, top-level and rule-level string variable values for `count` / `time_window`, `rules`, per-rule `header_prefix`, `key_type = var`, `constant`, and `var_combination`, HTTP header variables, quota headers, plugin metadata custom quota header names, `rejected_code`, APISIX-style empty/custom rejection bodies, and `allow_degradation`
  - not support shared `group` quotas, route-scoped Redis keys, or exact group-configuration mismatch validation
