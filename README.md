...
- [x] [elasticsearch-logger](https://apisix.apache.org/zh/docs/apisix/plugins/elasticsearch-logger/) 60%
  - support `endpoint_addr`, random `endpoint_addrs` selection, `field.index`, `auth`, `headers`, `timeout`, `ssl_verify`, `log_format`, official body-size config validation, and `_bulk` NDJSON delivery
  - not support APISIX batch processor behavior, `max_pending_entries`, Elasticsearch major-version discovery, time/APISIX variable expansion in `field.index`, or request/response body capture
