...
func TestBuildOutputsContainsOnlyPluginRegistry(t *testing.T) {
	manifest, err := capability.Load()
	if err != nil {
		t.Fatal(err)
	}

	outputs, err := buildOutputs(manifest)
	if err != nil {
		t.Fatal(err)
	}
	if len(outputs) != 1 {
		t.Fatalf("buildOutputs() returned %d outputs, want 1", len(outputs))
	}
	if outputs[0].relativePath != registryOutputPath {
		t.Fatalf("buildOutputs() path = %q, want %q", outputs[0].relativePath, registryOutputPath)
	}
}
