...
| `apisix.data_encryption` | Configures encrypted resource-field handling. New writes use `v2:` AES-GCM envelopes with a random 12-byte nonce and the canonical `plugin-name.field-path` as authenticated context. Unversioned AES-CBC remains decrypt-only for migration and is rewritten as v2 when it passes through the write path. Keep older keys after the newest key until legacy values have been rewritten. |
