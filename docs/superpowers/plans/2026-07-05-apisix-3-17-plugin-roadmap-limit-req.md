...
### Task 133: Implement `lago` Random Endpoint Selection
...
**Interfaces:**
- Consumes: official Lago `endpoint_addrs` list and `endpoint_uri`.
- Produces: per-send random Lago endpoint selection with stable request/error reporting for the selected endpoint.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/lago.lua`; APISIX builds `endpoint_addrs[math.random(#endpoint_addrs)] .. endpoint_uri` for delivery.
...
Updated `lago` support notes to include random `endpoint_addrs` selection and removed that item from the unsupported list.
...
### Task 138: Implement `http-logger` Request and Response Body Capture
...
- Produces: HTTP logger payloads with APISIX-style nested `request.body` and `response.body` fields while preserving the upstream request body stream and client response body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/http-logger.lua` and `apisix/utils/log-util.lua`; APISIX reads request body in access phase, collects response body in body filter, and adds bodies to the log entry when the include flags are enabled.

- [x] **Step 2: Write failing tests**

Test covers request body preservation for the upstream handler, response body preservation for the client, and emitted HTTP logger payloads containing nested `request.body` and `response.body`.

- [x] **Step 3: Implement body capture**

Added `http-logger` config fields for raw body include flags, default body byte caps, request body read/restore, response body recording while streaming to the original writer, and nested payload insertion.

- [x] **Step 4: Update README**

Updated `http-logger` support notes to include `include_req_body`, `include_resp_body`, and capped body-size capture, while keeping expression filters unsupported.

- [x] **Step 5: Verify**

Run: `go test ./pkg/plugin/http_logger -count=1 -timeout=15s -v`, `go test ./...`, and `make build`.

### Task 139: Implement `udp-logger` Request and Response Body Capture
...
**Interfaces:**
- Consumes: `include_req_body`, `include_resp_body`, `max_req_body_bytes`, and `max_resp_body_bytes` route/service plugin config.
- Produces: UDP logger payloads with APISIX-style nested `request.body` and `response.body` fields while preserving the upstream request body stream and client response body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/udp-logger.lua` and `apisix/utils/log-util.lua`; APISIX reads request body in access phase, collects response body in body filter, and adds bodies to the log entry when the include flags are enabled.

- [x] **Step 2: Write failing tests**

Test covers request body preservation for the upstream handler, response body preservation for the client, and emitted UDP logger payloads containing nested `request.body` and `response.body`.

- [x] **Step 3: Implement body capture**

Added `udp-logger` config fields for raw body include flags, default body byte caps, request body read/restore, response body recording while streaming to the original writer, and nested payload insertion.

- [x] **Step 4: Update README**

Updated `udp-logger` support notes to include `include_req_body`, `include_resp_body`, and capped body-size capture, while keeping expression filters unsupported.

- [x] **Step 5: Verify**

Run: `go test ./pkg/plugin/udp_logger -count=1 -timeout=15s -v`, `go test ./...`, and `make build`.

### Task 140: Implement `tcp-logger` Request and Response Body Capture
...
**Interfaces:**
- Consumes: `include_req_body`, `include_resp_body`, `max_req_body_bytes`, and `max_resp_body_bytes` route/service plugin config.
- Produces: TCP logger payloads with APISIX-style nested `request.body` and `response.body` fields while preserving the upstream request body stream and client response body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/tcp-logger.lua` and `apisix/utils/log-util.lua`; APISIX reads request body in access phase, collects response body in body filter, and adds bodies to the log entry when the include flags are enabled.

- [x] **Step 2: Write failing tests**

Tests cover request body preservation for the upstream handler, response body preservation for the client, emitted TCP logger payloads containing nested `request.body` and `response.body`, and schema acceptance for official body-size caps.

- [x] **Step 3: Implement body capture**

Added `tcp-logger` config/schema fields for raw body include flags and body byte caps, request body read/restore, response body recording while streaming to the original writer, and nested payload insertion.

- [x] **Step 4: Update README**

Updated `tcp-logger` support notes to include `include_req_body`, `include_resp_body`, and capped body-size capture, while keeping expression filters unsupported.
...
### Task 142: Implement `skywalking-logger` Body Capture
...
- Produces: SkyWalking log entries whose `body.json.json` payload includes APISIX-style nested `request.body` and `response.body` fields while preserving the upstream request body stream and client response body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/skywalking-logger.lua`; official schema includes `include_req_body`, and the Go implementation already exposed response-body and body-size config fields. This slice makes those existing Go fields effective.

- [x] **Step 2: Write failing tests**

Focused handler test initially failed because SkyWalking delivery happened but the nested `request` object was missing from the payload.

- [x] **Step 3: Implement body capture**

Added body byte cap defaults, request body read/restore, response body recording while streaming to the original writer, and nested body payload insertion before the entry is sent through the SkyWalking logger channel.

- [x] **Step 4: Update README**

Updated `skywalking-logger` support notes to include `include_req_body`, `include_resp_body`, and capped body-size capture, while keeping batch processor and `max_pending_entries` unsupported.
...
### Task 144: Implement `rocketmq-logger` Request and Response Body Capture
...
**Interfaces:**
- Consumes: `include_req_body`, `include_resp_body`, `max_req_body_bytes`, and `max_resp_body_bytes` route/service plugin config.
- Produces: RocketMQ logger messages with APISIX-style nested `request.body` and `response.body` fields while preserving the upstream request body stream and client response body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/rocketmq-logger.lua`; official schema includes request/response body flags, expression filters, and producer defaults. This slice implements the raw body flags for the default log-entry shape.

- [x] **Step 2: Write failing tests**

Focused handler test initially failed because RocketMQ delivery happened but the nested `request` object was missing from the JSON message.

- [x] **Step 3: Implement body capture**

Added a body-aware handler path for RocketMQ logger configs with raw body capture enabled, including request body read/restore, response body recording while streaming to the original writer, and nested payload insertion before sending through the logger channel.

- [x] **Step 4: Update README**

Updated `rocketmq-logger` support notes to include `include_req_body`, `include_resp_body`, and capped body-size capture, while keeping batch processor, `max_pending_entries`, expression filters, `meta_format = origin`, and `use_tls` unsupported.
...
### Task 147: Implement `tencent-cloud-cls` Request and Response Body Capture
...
**Interfaces:**
- Consumes: `include_req_body`, `include_resp_body`, `max_req_body_bytes`, and `max_resp_body_bytes` route/service plugin config.
- Produces: Tencent CLS protobuf log payloads with APISIX-style nested `request.body` and `response.body` fields while preserving the upstream request body stream and client response body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/tencent-cloud-cls.lua`; official schema includes request/response body flags, sampling, global tags, and log format support. This slice implements the raw body flags for the default log-entry shape.

- [x] **Step 2: Write failing tests**

Focused handler test initially failed because CLS delivery happened but the nested `request` JSON string field was empty.

- [x] **Step 3: Implement body capture**

Added a body-aware handler path for CLS logger configs with raw body capture enabled, including sampled request body read/restore, response body recording while streaming to the original writer, and nested payload insertion before sending through the logger channel.

- [x] **Step 4: Update README**

Updated `tencent-cloud-cls` support notes to include `include_req_body`, `include_resp_body`, and capped body-size capture, while keeping batch processor, `max_pending_entries`, expression filters, and lz4/zstd compression unsupported.
...
### Task 185: Align `limit-req` Custom Rejection Body
...
**Interfaces:**
- Consumes: official APISIX 3.17 `limit-req` custom `rejected_msg` behavior.
...
Read official APISIX 3.17 `apisix/plugins/limit-req.lua`; when `conf.rejected_msg` is set and a request is rejected, APISIX returns `conf.rejected_code` with an `error_msg` JSON body.

- [x] **Step 2: Add focused test**

Updated the existing rejected-message test to require `application/json` and body `{"error_msg":"slow down"}`. Verified the test failed first against the old `http.Error` text response.

- [x] **Step 3: Implement JSON rejection body**

Precomputed the APISIX-style `error_msg` JSON body during `PostInit` and wrote it directly on rejection; default rejections now write only the configured status code.

- [x] **Step 4: Update README**

Updated `limit-req` support notes to include APISIX-style empty/custom rejection bodies while keeping Redis and Redis Cluster as documented gaps.

- [x] **Step 5: Verify**

Run: `go test ./pkg/plugin/limit_req -run TestHandlerUsesRejectedMessage -count=1 -timeout=10s -v` and `go test ./pkg/plugin/limit_req -count=1 -timeout=10s -v`. Full verification remains `go test ./...`, `make build`, and `git diff --check`.

### Task 186: Align `limit-conn` Custom Rejection Body
...
**Interfaces:**
- Consumes: official APISIX 3.17 `limit-conn` custom `rejected_msg` behavior.
- Produces: rejected requests with `rejected_msg` returning JSON `{"error_msg": "<message>"}` and `Content-Type: application/json`, while default rejections keep an empty body.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/limit-conn/init.lua`; when `conf.rejected_msg` is set and a connection is rejected, APISIX returns `conf.rejected_code` with an `error_msg` JSON body.

- [x] **Step 2: Add focused test**

Updated the existing rejected-message test to require `application/json` and body `{"error_msg":"too many connections"}`. Verified the test failed first against the old `http.Error` text response.
...
Updated `limit-conn` support notes to include APISIX-style empty/custom rejection bodies while keeping expression values, delay mode, Redis, and Redis Cluster as documented gaps.
...
### Task 190: Support Rule-Level `limit-count` String Limits
...
**Interfaces:**
- Consumes: official APISIX 3.17 `limit-count` `rules[].count` and `rules[].time_window` values as either integers or strings resolved from request variables.
- Produces: rule-level `limit-count` configs that validate and parse string variable expressions such as `$http_x_limit`, resolve them per request, and keep per-rule quota headers.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/limit-count/init.lua`; rule-level `count` and `time_window` are resolved per request, and rules whose limits or keys cannot resolve are skipped.

- [x] **Step 2: Add focused test**

Added a config-path test proving string rule `count` / `time_window` values pass schema validation, parse through `util.Parse`, resolve from request headers, reject the second request, and emit the configured per-rule remaining quota header.

- [x] **Step 3: Implement rule-level string values**

Changed rule config storage to preserve numeric or string values, prebuilt static rule limiters when possible, resolved dynamic rule limits per request, and reused the existing dynamic limiter cache for resolved rule limits.

- [x] **Step 4: Update README**

Updated `limit-count` support notes to include top-level and rule-level string variable values for `count` / `time_window`; `redis-cluster` remains the documented gap.
...
### Task 198: Support `udp-logger` Body Expression Gates
...
**Interfaces:**
- Consumes: official APISIX 3.17 `udp-logger` `include_req_body_expr` and `include_resp_body_expr` config.
- Produces: UDP log entries whose request and response body fields are gated by bounded request-variable expressions while preserving request body replay, capped capture, and UDP delivery behavior.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/udp-logger.lua` and `apisix/utils/log-util.lua`; `udp-logger` uses the shared log utility to evaluate request and response body expression gates only when the corresponding body capture flag is enabled.

- [x] **Step 2: Add focused tests**

Added tests proving matching expressions capture request and response bodies in the JSON payload delivered over UDP, while non-matching expressions omit both logged bodies without preventing downstream from reading the original request body.

- [x] **Step 3: Implement bounded expression gates**

Added config fields for `include_req_body_expr` / `include_resp_body_expr`, gated request body reads before downstream execution, captured response status in the UDP logger recorder, and evaluated response body logging after downstream completion using the existing local `==`, `!=`, numeric comparison, regex, `AND`, and `OR` expression subset.

- [x] **Step 4: Update README**

Updated `udp-logger` support notes to include body expression gates; APISIX batch processor behavior and `max_pending_entries` remain documented gaps.

- [x] **Step 5: Verify**

Run: `go test ./pkg/plugin/udp_logger -run 'TestHandler(IncludesBodiesWhenExpressionsMatch|SkipsBodiesWhenExpressionsDoNotMatch)' -count=1 -timeout=10s -v` and `go test ./pkg/plugin/udp_logger -count=1 -timeout=10s -v`. Full verification remains `go test ./...`, `make build`, and `git diff --check`.

### Task 199: Support `kafka-logger` Body Expression Gates
...
**Interfaces:**
- Consumes: official APISIX 3.17 `kafka-logger` `include_req_body_expr` and `include_resp_body_expr` config.
- Produces: Kafka log entries whose request and response body fields are gated by bounded request-variable expressions while preserving request body replay, capped capture, and existing broker/sender behavior.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/kafka-logger.lua` and `apisix/utils/log-util.lua`; `kafka-logger` validates body expression configs through the shared log utility and evaluates request/response body gates only when the corresponding body capture flag is enabled.

- [x] **Step 2: Add focused tests**

Added tests proving matching expressions capture request and response bodies in the Kafka message payload, while non-matching expressions omit both logged bodies without preventing downstream from reading the original request body.

- [x] **Step 3: Implement bounded expression gates**

Gated request body reads before downstream execution, captured response status in the Kafka logger recorder, and evaluated response body logging after downstream completion using the existing local `==`, `!=`, numeric comparison, regex, `AND`, and `OR` expression subset.

- [x] **Step 4: Update README**

Updated `kafka-logger` support notes to include body expression gates; APISIX batch processor behavior, `max_pending_entries`, and `meta_format = origin` remain documented gaps.

- [x] **Step 5: Verify**

Run: `go test ./pkg/plugin/kafka_logger -run 'TestHandler(IncludesBodiesWhenExpressionsMatch|SkipsBodiesWhenExpressionsDoNotMatch)' -count=1 -timeout=10s -v` and `go test ./pkg/plugin/kafka_logger -count=1 -timeout=10s -v`. Full verification remains `go test ./...`, `make build`, and `git diff --check`.

### Task 200: Support `rocketmq-logger` Body Expression Gates
...
**Interfaces:**
- Consumes: official APISIX 3.17 `rocketmq-logger` `include_req_body_expr` and `include_resp_body_expr` config.
- Produces: RocketMQ log entries whose request and response body fields are gated by bounded request-variable expressions while preserving request body replay, capped capture, and existing sync producer delivery behavior.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/rocketmq-logger.lua`; `rocketmq-logger` delegates request/response body capture to the shared log utility and validates body expression configs through `log_util.check_log_schema`.

- [x] **Step 2: Add focused tests**

Added tests proving matching expressions capture request and response bodies in the RocketMQ message payload, while non-matching expressions omit both logged bodies without preventing downstream from reading the original request body.

- [x] **Step 3: Implement bounded expression gates**

Gated request body reads before downstream execution, captured response status in the RocketMQ logger recorder, and evaluated response body logging after downstream completion using the existing local `==`, `!=`, numeric comparison, regex, `AND`, and `OR` expression subset.

- [x] **Step 4: Update README**

Updated `rocketmq-logger` support notes to include body expression gates; APISIX batch processor behavior, `max_pending_entries`, `meta_format = origin`, and `use_tls` remain documented gaps.
...
### Task 202: Support `syslog` Body Expression Gates
...
**Interfaces:**
- Consumes: official APISIX 3.17 `syslog` `include_req_body_expr` and `include_resp_body_expr` config.
- Produces: syslog JSON payloads whose request and response body fields are gated by bounded request-variable expressions while preserving request body replay, capped capture, and existing UDP/TCP delivery behavior.

- [x] **Step 1: Read official behavior**

Read official APISIX 3.17 `apisix/plugins/syslog.lua` and `apisix/utils/log-util.lua`; `syslog` declares the request/response body expression fields and delegates body capture to the shared log utility.

- [x] **Step 2: Add focused tests**

Added tests proving matching expressions capture request and response bodies in the syslog UDP payload, while non-matching expressions omit both logged bodies without preventing downstream from reading the original request body.

- [x] **Step 3: Implement bounded expression gates**

Enabled config fields for `include_req_body_expr` / `include_resp_body_expr`, gated request body reads before downstream execution, captured response status in the syslog recorder, and evaluated response body logging after downstream completion using the existing local `==`, `!=`, numeric comparison, regex, `AND`, and `OR` expression subset.

- [x] **Step 4: Update README**

Updated `syslog` support notes to include body expression gates; APISIX batch processor buffering and OpenResty connection pooling/TLS behavior remain documented gaps.
...
### Task 206: Support `loki-logger` Body Expression Gates
...
**Interfaces:**
- Consumes: APISIX-compatible `loki-logger` `include_req_body_expr` and `include_resp_body_expr` config.
- Produces: Loki stream entries whose request and response body fields are gated by bounded request-variable expressions while preserving request body replay, capped capture, random endpoint selection, tenant headers, and existing `/loki/api/v1/push` delivery behavior.
...
Added schema/config fields for `include_req_body_expr` / `include_resp_body_expr`, gated request body reads before downstream execution, captured response status in the Loki logger recorder, and evaluated response body logging after downstream completion using the existing local `==`, `!=`, numeric comparison, regex, `AND`, and `OR` expression subset.

- [x] **Step 3: Update docs**

Updated `loki-logger` README support notes and the live APISIX 3.17 parity checklist.

- [x] **Step 4: Verify**

Run: `go test ./pkg/plugin/loki_logger -run 'TestHandler(IncludesBodiesWhenExpressionsMatch|SkipsBodiesWhenExpressionsDoNotMatch)|TestSchemaAcceptsOfficialBodyExpressionFields' -count=1 -timeout=10s -v` and `go test ./pkg/plugin/loki_logger -count=1 -timeout=10s -v`. Full verification remains `go test ./...`, `make build`, and `git diff --check`.

### Task 207: Support `loggly` Body Expression Gates
...
**Interfaces:**
- Consumes: official APISIX 3.17 `loggly` `include_req_body_expr` and `include_resp_body_expr` config.
- Produces: Loggly UDP syslog or HTTP/S bulk log entries whose request and response body fields are gated by bounded request-variable expressions while preserving request body replay, capped capture, severity mapping, tags, and existing delivery behavior.

- [x] **Step 1: Confirm official fields**

Read `origin/release/3.17:apisix/plugins/loggly.lua`; the official schema includes both `include_req_body_expr` and `include_resp_body_expr`.
...
Added schema/config fields for `include_req_body_expr` / `include_resp_body_expr`, gated request body reads before downstream execution, captured response status in the Loggly recorder, and evaluated response body logging after downstream completion using the existing local `==`, `!=`, numeric comparison, regex, `AND`, and `OR` expression subset.

- [x] **Step 4: Update docs**

Updated `loggly` README support notes and the live APISIX 3.17 parity checklist.
...
### Task 210: Support `key-auth` Anonymous Consumer Fallback
...
**Interfaces:**
- Consumes: official APISIX 3.17 `key-auth` `anonymous_consumer` config.
- Produces: failed or missing API-key authentication can attach the configured anonymous consumer and continue; invalid presented credentials are stripped before fallback when `hide_credentials` is enabled.

- [x] **Step 1: Add focused failing tests**

Added tests for missing-key fallback, invalid-key fallback with credential stripping, and schema acceptance. The first focused run failed because `Config` did not yet expose `AnonymousConsumer`.

- [x] **Step 2: Implement fallback**

Added schema/config support for `anonymous_consumer`, looked up the anonymous consumer with `store.GetConsumer`, attached it with the existing APISIX context helper, and removed both header and query credentials before invalid-key fallback when `hide_credentials` is enabled.

- [x] **Step 3: Update docs**

Updated `key-auth` README support notes and the live APISIX 3.17 parity checklist.
...
### Task 225: Support OpenAI Chat `ai-proxy-multi` Request Body Overrides
...
**Interfaces:**
- Consumes: APISIX `ai-proxy-multi` per-instance `override.request_body.openai-chat` and `override.request_body_force_override`.
- Produces: recursive request-body merge for the selected local OpenAI Chat-compatible instance, filling missing fields by default and overwriting client fields when force mode is enabled.

- [x] **Step 1: Confirm current gap**

Local `ai-proxy-multi` exposed `override.request_body` config fields but did not apply them while constructing the per-instance provider request body.

- [x] **Step 2: Add focused failing tests**

Added default-merge and force-merge tests for a selected instance. The first focused run failed because missing fields were not filled and force mode did not overwrite client fields.

- [x] **Step 3: Implement per-instance OpenAI Chat override merge**

Added local merge support for `openai-chat` request-body overrides plus a direct-map fallback for existing simplified configs. Nested maps are recursively merged; arrays and scalars are replaced only for missing fields or force mode.

- [x] **Step 4: Update docs**

Updated `ai-proxy-multi` README support notes and the live APISIX 3.17 parity checklist.

- [x] **Step 5: Verify**

Run: `go test ./pkg/plugin/ai_proxy_multi -run 'TestHandler(MergesRequestBodyOverrideWithoutForce|ForceMergesRequestBodyOverride)' -count=1 -timeout=10s -v` and `go test ./pkg/plugin/ai_proxy_multi -count=1 -timeout=10s -v`. Full verification remains `go test ./...`, `make build`, and `git diff --check`.

### Task 226: Omit `model` for Azure OpenAI Provider Requests
...
- Produces: `ai-proxy` and `ai-proxy-multi` provider request bodies that omit `model` when `provider = "azure-openai"`.

- [x] **Step 1: Confirm official behavior**

Read APISIX 3.17 Azure OpenAI provider source; the provider sets `remove_model = true`, so model is not sent in the outgoing JSON request body.

- [x] **Step 2: Add focused failing tests**

Added focused tests for `ai-proxy` and `ai-proxy-multi` that configure Azure OpenAI with a client/body model and option model. The first focused runs failed because `model` was forwarded as `gpt-4`.

- [x] **Step 3: Implement provider body rule**

Added a small provider-specific body rule in both AI proxy packages to delete `model` after request body overrides, options, and LLM option shaping are applied.
