...
|---|---:|---|
| `http-logger` | 76% | Shared batch processor, `max_pending_entries`, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population, stale-object cleanup exactness, and encrypted `auth_header`. |
| `skywalking-logger` | 76% | Shared batch processor, `max_pending_entries`, SkyWalking JSON-array batch payloads, basic `sw8` trace correlation, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population and stale-object cleanup exactness. |
| `tcp-logger` | 70% | Shared batch processor, `max_pending_entries`, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are OpenResty cosocket pooling, APISIX batch `server_addr` label population, and stale-object cleanup exactness. |
...
| `syslog` | 70% | Shared batch processor, `max_pending_entries`, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are OpenResty syslog connection pooling/TLS behavior parity, APISIX batch `server_addr` label population, and stale-object cleanup exactness. |
| `udp-logger` | 70% | Shared batch processor, `max_pending_entries`, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population and stale-object cleanup exactness. |
| `clickhouse-logger` | 76% | Shared batch processor, `max_pending_entries`, JSONEachRow batch payloads, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population, stale-object cleanup exactness, and encrypted `password`. |
...
| `sls-logger` | 72% | Shared batch processor, concatenated RFC5424 batch writes, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population, stale-object cleanup exactness, and encrypted `access_key_secret`. |
...
| `loggly` | 76% | Shared batch processor, HTTP/S newline bulk batching, UDP per-entry batch delivery, metadata delivery config fallback, `max_pending_entries`, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population, stale-object cleanup exactness, and encrypted `customer_token`. |
| `elasticsearch-logger` | 84% | Shared batch processor, `max_pending_entries`, `_bulk` NDJSON batch delivery, and route-aware `batch_process_entries` gauge hook are implemented while preserving index expansion, auth, headers, and body-expression behavior. Remaining gaps are APISIX batch `server_addr` label population, stale-object cleanup exactness, and encrypted `auth.password`. |
| `loki-logger` | 76% | Shared batch processor, `max_pending_entries`, one-stream multi-value Loki batches, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population and stale-object cleanup exactness. |
| `tencent-cloud-cls` | 76% | Shared batch processor, `max_pending_entries`, multi-log protobuf batch payloads, and route-aware `batch_process_entries` gauge hook are implemented. Remaining gaps are APISIX batch `server_addr` label population, stale-object cleanup exactness, and encrypted `secret_key`. The upstream APISIX 3.17 SDK has an lz4/zstd TODO but no plugin config/feature to match. |
