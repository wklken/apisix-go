...
**Interfaces:**
- Produces: explicit `$encrypted://v2:` AES-GCM envelopes with random 12-byte nonces and authenticated canonical field context; `ResolveForContext` for strict fields.
...
- [x] **Step 3: Seal with AES-GCM and `crypto/rand`; encode explicit `$encrypted://v2:` plus base64(nonce || ciphertext); use canonical `plugin-name.field-path` AAD**
