...
**Secret-domain amendment:** The worker and supervisor retain closure grants separately for every publication domain. Each `SecretRequest.Scope.Domain` is mandatory, and authorization is keyed by `(Domain, ResourceKey)` so independently recovered HTTP and stream publications with the same resource key but different bytes cannot borrow one another's authority.
...
func TestWorkerBuildsCompilerLocallyAndRequestsOnlyScopedSecrets(t *testing.T) {
	...
	wantScope := secret.Scope{Generation: 42, Attempt: wantAttempt, Domain: generation.DomainHTTP, Plugin: "key-auth",
