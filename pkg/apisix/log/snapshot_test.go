...
func TestValueFromSnapshotCoversFallbackVariableSemantics(t *testing.T) {
	...
	snapshot := LogSnapshot{
		Request: RequestLogSnapshot{
			...
			RequestVars: map[string]any{
				"$shared": "request", "$request_only": "value",
				"$request_length": int64(108), "$bytes_sent": int64(133),
			},
	...
	want := map[string]any{
		...
		"status_code": http.StatusTeapot, "request_length": int64(108), "bytes_sent": int64(133),
