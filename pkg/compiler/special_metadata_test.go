...
func TestSpecialMetadataOTelAliasStaysDistinctInCompiler(t *testing.T) {
	compiler := newTestCompiler(t)
	...
	for _, test := range []struct {
	...
	}{
		{
			name:     "N",
			revision: 705,
			document: `{"trace_id_source":"random","collector":{"address":"otel-n:4318"}}`,
		},
		{
			name:     "N+1",
			revision: 706,
			document: `{"trace_id_source":"x-request-id","collector":{"address":"otel-n1:4318"}}`,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			snapshot := mustGenerationSnapshot(t, test.revision, []generation.Resource{
				resourceValue("plugin_metadata", "otel", test.document),
			}, nil)
			...
			candidate := set.Domains[generation.DomainHTTP]
			got, found := candidate.Snapshot.Lookup(generation.ResourceKey{Kind: "plugin_metadata", ID: "otel"})
			if !found || !bytes.Equal(got, []byte(test.document)) {
				t.Fatalf("alias metadata bytes = %q/%v, want %q", got, found, test.document)
			}
			if _, found := candidate.Snapshot.Lookup(
