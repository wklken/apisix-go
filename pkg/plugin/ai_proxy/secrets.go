...
type aiProxySecretState struct {
...
type stagedAIProxySecrets struct {
	headerNames       []string
	headers           map[string]secret.Value
...
type aiProxySecretSnapshot struct {
	headerNames []string
	headers     map[string]secret.Value
	queryNames  []string
	queries     map[string]secret.Value
...
func (p *Plugin) MaterializeScopedSecrets(
	...
	staged := stagedAIProxySecrets{
		headerNames:       sortedAuthNames(p.config.Auth.Header),
		...
		queryNames:        sortedAuthNames(p.config.Auth.Query),
	...
	var err error
	for _, header := range staged.headerNames {
		staged.headers[header], err = access.Materialize(ctx, "auth.header", p.config.Auth.Header[header])
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
		staged.headerDescriptors[header], err = aiProxyDescriptor(staged.headers[header])
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
	}
	for _, key := range staged.queryNames {
		staged.queries[key], err = access.Materialize(ctx, "auth.query", p.config.Auth.Query[key])
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
		staged.queryDescriptors[key], err = aiProxyDescriptor(staged.queries[key])
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
	}
	if p.config.Auth.GCP != nil && p.config.Auth.GCP.ServiceAccountJSON != "" {
		staged.gcp, err = access.Materialize(
			ctx, "auth.gcp.service_account_json", p.config.Auth.GCP.ServiceAccountJSON,
		)
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
	...
	if p.config.Auth.AWS != nil && p.config.Auth.AWS.SecretAccessKey != "" {
		staged.awsSecret, err = access.Materialize(
			ctx, "auth.aws.secret_access_key", p.config.Auth.AWS.SecretAccessKey,
		)
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
		staged.awsDescriptor, err = aiProxyDescriptor(staged.awsSecret)
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
		staged.hasAWS = true
	...
	if p.config.Auth.AWS != nil && p.config.Auth.AWS.SessionToken != "" {
		staged.awsSession, err = access.Materialize(
			ctx, "auth.aws.session_token", p.config.Auth.AWS.SessionToken,
		)
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
		staged.sessionDescriptor, err = aiProxyDescriptor(staged.awsSession)
		if err != nil {
			return secret.ErrCredentialUnavailable
		}
...
func sortedAuthNames(values map[string]string) []string {
...
func aiProxyDescriptor(value secret.Value) (string, error) {
...
func (p *Plugin) installScopedSecrets(staged stagedAIProxySecrets) error {
	state := &p.secrets
	state.credentialMu.Lock()
	defer state.credentialMu.Unlock()
	if state.retired {
...
func (p *Plugin) withAuth(use func(Auth) error) error {
	...
	if !prepared {
		auth := clonePlainAuth(p.config.Auth)
		defer clearAuth(&auth)
		return use(auth)
	}
	defer release()
	auth := cloneAuthShape(p.config.Auth)
	defer clearAuth(&auth)
...
type aiProxySecretEntry struct {
	value secret.Value
	kind  uint8
	name  string
}
...
func (snapshot aiProxySecretSnapshot) entries() []aiProxySecretEntry {
	entries := make([]aiProxySecretEntry, 0, len(snapshot.headerNames)+len(snapshot.queryNames)+3)
	for _, name := range snapshot.headerNames {
		entries = append(entries, aiProxySecretEntry{value: snapshot.headers[name], kind: secretHeader, name: name})
	}
	for _, name := range snapshot.queryNames {
		entries = append(entries, aiProxySecretEntry{value: snapshot.queries[name], kind: secretQuery, name: name})
	}
	if snapshot.hasGCP {
		entries = append(entries, aiProxySecretEntry{value: snapshot.gcp, kind: secretGCP})
	}
	if snapshot.hasAWS {
		entries = append(entries, aiProxySecretEntry{value: snapshot.awsSecret, kind: secretAWS})
	}
	if snapshot.hasSession {
		entries = append(entries, aiProxySecretEntry{value: snapshot.awsSession, kind: secretAWSSession})
	}
	return entries
}

func (entry aiProxySecretEntry) set(auth *Auth, value string) {
	switch entry.kind {
	case secretHeader:
	...
	case secretAWSSession:
		auth.AWS.SessionToken = value
	}
}

func (p *Plugin) acquireSecretSnapshot() (aiProxySecretSnapshot, func(), bool, error) {
	state := &p.secrets
	state.credentialMu.Lock()
	defer state.credentialMu.Unlock()
	if state.retired {
		return aiProxySecretSnapshot{}, nil, false, secret.ErrCredentialUnavailable
	}
	if !state.prepared {
		return aiProxySecretSnapshot{}, nil, false, nil
	}
	if state.activeUses == 0 {
		state.usesDone = make(chan struct{})
	}
	state.activeUses++
	return aiProxySecretSnapshot{
		headerNames: append([]string(nil), state.headerNames...), headers: state.headers,
		queryNames: append([]string(nil), state.queryNames...), queries: state.queries,
		gcp: state.gcp, hasGCP: state.hasGCP,
		awsSecret: state.awsSecret, hasAWS: state.hasAWS,
		awsSession: state.awsSession, hasSession: state.hasSession,
...
func clonePlainAuth(source Auth) Auth {
	auth := source
...
func cloneAuthShape(source Auth) Auth {
	auth := clonePlainAuth(source)
