...
func TestRunRequestPhasePublishesAuthenticationStateAndLegacyHandlerCallsNextOnce(t *testing.T) {
	addKeyAuthConsumer(t, "phase-key-user", "phase-key")
	p := newTestPlugin(t, Config{})
	request := httptest.NewRequest(http.MethodGet, "http://example.com/", nil)
	request = ctx.WithApisixVars(request, map[string]string{})
	request.Header.Set("apikey", "phase-key")
	...
	if !ok || state.Source != name || state.Consumer().Username != "phase-key-user" {
		t.Fatalf("authentication state = (%+v, %v), want key-auth/phase-key-user", state, ok)
	}
