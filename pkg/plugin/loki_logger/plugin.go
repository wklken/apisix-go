...
func (p *Plugin) PostInit() error {
	...
	processor, err := base.NewBatchProcessor("loki logger", p.TaskOwner(), base.BatchDefaults{
		PluginID:           name,
...
func lokiSnapshotDefaultFields(snapshot base.LogSnapshot, requestStart time.Time) map[string]any {
	...
	fields := map[string]any{
		"request": map[string]any{
			...
			"headers":     base.CollapseAccessLogHeaderValues(snapshot.Request.Header),
			"querystring": base.CollapseHeaderValues(http.Header(snapshot.Request.Query)),
			"size":        requestSize,
		},
		"response": map[string]any{
			"status":  snapshot.Outcome.Status,
			"headers": base.CollapseAccessLogHeaderValues(snapshot.Response.Header),
