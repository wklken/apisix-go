...
func (p *Plugin) Handler(next http.Handler) http.Handler {
	...
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		writer := w
		var recorder *tcpLogResponseRecorder
		if p.config.IncludeRespBody {
			recorder = &tcpLogResponseRecorder{
				ResponseWriter: w,
...
type tcpLogResponseRecorder struct {
	http.ResponseWriter
	body   bytes.Buffer
	limit  int
	status int
...
func (w *tcpLogResponseRecorder) WriteHeader(status int) {
	w.status = status
	w.ResponseWriter.WriteHeader(status)
}

func (w *tcpLogResponseRecorder) Write(body []byte) (int, error) {
	if w.status == 0 {
...
func (w *tcpLogResponseRecorder) capture(body []byte) {
