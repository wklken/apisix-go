...
test -x "$smoke"
bash -n "$smoke"
require_pattern 'container_bin=\$\{CONTAINER_BIN:-docker\}' "$smoke"
require_pattern 'command "\$container_bin"' "$smoke"
require_pattern 'probe_mode=\$\{CONTAINER_SMOKE_PROBE_MODE:-auto\}' "$smoke"
require_pattern 'podman-machine' "$smoke"
require_pattern 'docker machine ssh -- curl' "$smoke"
...
require_pattern '--publish 127\.0\.0\.1::7085' "$smoke"
require_pattern 'status_deadline=' "$smoke"
require_pattern '/status' "$smoke"
require_pattern 'ready_deadline=' "$smoke"
require_pattern '/status/ready' "$smoke"
