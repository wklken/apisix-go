...
func TestCompareDifferentialDatadogRequiresSixOrderedSingleMetricDatagrams(t *testing.T) {
	spec := differentialCasesForPlugin("datadog")[0]
...
func TestCompareDifferentialDatadogAcceptsFiniteNonnegativeScientificLatency(t *testing.T) {
	spec := differentialCasesForPlugin("datadog")[0]
	candidate, oracle := differentialDatadogComparatorObservations(spec)
	oracle.UpstreamCalls[3].Body = strings.Replace(
		oracle.UpstreamCalls[3].Body,
		":2.25|h",
		":8.392333984375e-05|h",
		1,
	)
	...
	if err != nil || !passed || diff != "" {
		t.Fatalf("compare scientific Datadog latency = %t, %q, %v", passed, diff, err)
	}
}
