...
func compareDifferentialCaseObservations(
	...
	if err := validateDifferentialObservationExpectation("candidate", spec.Expect, left); err != nil {
		return false, "", err
	}
	if err := validateDifferentialObservationExpectation("oracle", spec.Expect, right); err != nil {
		return false, "", err
	}
...
func validateDifferentialObservationExpectation(
	side string,
	...
	if expectation.Status != nil && observation.Status != *expectation.Status {
		return fmt.Errorf(
			"%s expectation status = %d, got %d",
			side,
			*expectation.Status,
			observation.Status,
		)
	}
	if expectation.Body != nil && observation.Body != *expectation.Body {
		return fmt.Errorf("%s expectation body differs", side)
	}
	if expectation.BodySHA256 != "" &&
		differentialStringSHA256(observation.Body) != expectation.BodySHA256 {
		return fmt.Errorf("%s expectation body_sha256 differs", side)
	}
	if expectation.UpstreamReceived != nil &&
		observation.Upstream.Received != *expectation.UpstreamReceived {
		return fmt.Errorf(
			"%s expectation upstream_received = %t, got %t",
			side,
			*expectation.UpstreamReceived,
			observation.Upstream.Received,
		)
	}
	if expectation.UpstreamMethod != "" && observation.Upstream.Method != expectation.UpstreamMethod {
		return fmt.Errorf(
			"%s expectation upstream_method = %q, got %q",
			side,
			expectation.UpstreamMethod,
			observation.Upstream.Method,
		)
	}
	if expectation.UpstreamPath != "" && observation.Upstream.Path != expectation.UpstreamPath {
		return fmt.Errorf(
			"%s expectation upstream_path = %q, got %q",
			side,
			expectation.UpstreamPath,
			observation.Upstream.Path,
		)
	}
	if expectation.UpstreamBody != nil && observation.Upstream.Body != *expectation.UpstreamBody {
		return fmt.Errorf("%s expectation upstream_body differs", side)
	}
	if expectation.UpstreamBodySHA256 != "" &&
		differentialStringSHA256(observation.Upstream.Body) != expectation.UpstreamBodySHA256 {
		return fmt.Errorf("%s expectation upstream_body_sha256 differs", side)
	}
	for label, headers := range map[string]struct {
		expected map[string]string
		actual   map[string][]string
	}{
		"response_headers": {expectation.ResponseHeaders, observation.Headers},
		"upstream_headers": {expectation.UpstreamHeaders, observation.Upstream.Headers},
		"route_observer":   {expectation.RouteObserver, observation.RouteObserver},
	} {
		for name, expected := range headers.expected {
			values := differentialHeaderValues(headers.actual, name)
			if len(values) != 1 || values[0] != expected {
				return fmt.Errorf(
					"%s expectation %s[%q] = %q, got %v",
					side,
					label,
					name,
					expected,
...
func differentialStringSHA256(value string) string {
