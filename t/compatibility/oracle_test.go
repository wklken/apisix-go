...
func TestDecodeDifferentialCatalogOwnsRuntimeCase(t *testing.T) {
	path := filepath.Join(t.TempDir(), "cases.yaml")
	data := []byte(`schema_version: 3
suite: apisix-3.17-plugin-differential-v1
cases:
  - case: key-auth-missing
    plugin: key-auth
    obligation: deny-missing-api-key
    route_id: differential-key-auth-missing
    expect: {status: 401}
    config: {routes: [{id: differential-key-auth-missing, uri: /hello, plugins: {key-auth: {}}}]}
    request: {method: GET, path: /hello, host: gateway.example.test}
    fixture: {name: primary, expected_calls: 0, response: {status: 200, body: unused}}
...
`)
	...
	if err := decodeDifferentialCatalog(path, data, &catalog); err != nil {
		t.Fatal(err)
	}
	catalog.RequiredPlugins = differentialRequiredPlugins(catalog.Cases)
	if err := validateDifferentialCatalog(catalog); err != nil {
		t.Fatal(err)
	}
	if len(catalog.Cases) != 1 || catalog.Cases[0].Name != "key-auth-missing" ||
		catalog.Cases[0].Obligation != "deny-missing-api-key" ||
		catalog.Cases[0].Request.Host != "gateway.example.test" {
		t.Fatalf("loaded cases = %#v", catalog.Cases)
	}
}

func TestLoadDifferentialCatalogRejectsTruncatedPinnedSuite(t *testing.T) {
	path := filepath.Join(t.TempDir(), "cases.yaml")
	data := []byte(`schema_version: 3
suite: apisix-3.17-plugin-differential-v1
cases:
...
`)
	...
	if _, err := loadDifferentialCatalog(path); err == nil ||
		!strings.Contains(err.Error(), "cases = 1, want 124") {
		t.Fatalf("loadDifferentialCatalog() error = %v, want pinned case-count rejection", err)
	}
}

func TestDifferentialCatalogRejectsMissingDeclaredPluginBinding(t *testing.T) {
	catalog := DifferentialCatalog{
		SchemaVersion:   3,
		Suite:           differentialSuite,
		RequiredPlugins: []string{"traffic-label"},
		Cases: []DifferentialCase{{
			Name:       "traffic-label-missing-binding",
			Plugin:     "traffic-label",
			Obligation: "rewrite-request-header",
			RouteID:    "traffic-label-route",
			Expect:     &DifferentialExpectation{UpstreamReceived: new(bool)},
			Config: map[string]any{"routes": []any{map[string]any{
				"id": "traffic-label-route", "uri": "/hello", "plugins": map[string]any{},
			}}},
		}},
	}
	if err := validateDifferentialCatalog(catalog); err == nil ||
		!strings.Contains(err.Error(), "does not bind declared plugin") {
		t.Fatalf("validateDifferentialCatalog() error = %v, want missing-binding rejection", err)
	}
}

func TestDifferentialExpectationRejectsSharedTrafficLabelNoop(t *testing.T) {
	spec := mustDifferentialCase("traffic-label-overrides-request-header")
	if spec.Expect == nil || spec.Expect.RouteObserver["X-Route-Observer"] != "100" {
		t.Fatalf("traffic-label expectation = %#v", spec.Expect)
	}
	noop := DifferentialObservation{
		Status:        200,
		RouteObserver: map[string][]string{"X-Route-Observer": {"200"}},
	...
	if err == nil || passed || !strings.Contains(err.Error(), "candidate expectation") {
		t.Fatalf(
			"shared no-op comparison = passed %t error %v, want candidate expectation failure",
			passed,
			err,
		)
	}
...
func TestExactDifferentialCasesDeclareSemanticExpectation(t *testing.T) {
	var missing []string
	for _, spec := range differentialCases() {
		if spec.ComparisonPolicy == "" && spec.Expect == nil {
			missing = append(missing, spec.Name)
		}
	}
	if len(missing) != 0 {
		t.Fatalf("exact differential cases without semantic expect = %v", missing)
	}
}
...
func TestDifferentialCatalogDeclaresOneHundredTwentyFourCaseOneHundredElevenPluginSuite(t *testing.T) {
	...
	if catalog.SchemaVersion != 3 || catalog.Suite != differentialSuite {
		t.Fatalf(
	...
	for _, row := range catalog.Cases {
		if row.Plugin == "" || row.Obligation == "" || row.Name == "" {
...
func TestSelectedDifferentialArtifactRecordsExactSelectionAndFailures(t *testing.T) {
	...
	fullCatalog, err := loadDifferentialCatalog(differentialCatalogPath(repoRoot))
	if err != nil {
		t.Fatalf("loadDifferentialCatalog() error = %v", err)
	}
	selected, normalized, err := selectDifferentialCases(all, DifferentialSelection{
...
func TestDifferentialCompressedResponseComparisonDecodesEachSideStrictly(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.plugin, func(t *testing.T) {
			...
			if test.plugin == "gzip" {
				spec = differentialCasesForPlugin("gzip")[0]
			} else {
				spec = differentialCasesForPlugin("brotli")[0]
			}
...
func TestDifferentialChaitinWAFComparisonScopesOnlyElapsedTime(t *testing.T) {
	spec := differentialCasesForPlugin("chaitin-waf")[0]
...
func TestDifferentialLimitReqComparisonScopesOnlyPinned503Representation(t *testing.T) {
	spec := differentialCasesForPlugin("limit-req")[1]
	candidate := DifferentialObservation{
...
func TestDifferentialCSRFCookieComparisonNormalizesOnlyDynamicTokenFields(t *testing.T) {
	policy := testNormalizationPolicy()
	spec := differentialCasesForPlugin("csrf")[0]
	const key = "userkey"
...
func differentialCASAuthCallbackNosniffTestObservations() (
	...
	spec := differentialCasesForPlugin("cas-auth")[0]
...
func differentialLimitCountFixedWindowTestObservations() (
	...
	const oracle503Body = "<html>\r\n" +
		...
		"</html>\r\n"
	spec := differentialCasesForPlugin("limit-count")[0]
