...
- Each `<plugin>.yaml` manifest owns its executable standalone cases. Manifests
  contain no upstream source-accounting metadata or coverage ledger.
...
- Compare upstream behavior during development, then encode the resulting
  behavior in executable cases or the owning unit tests. Upstream source labels
  and metadata do not belong in manifests.
