...
func TestDifferentialKafkaProxyCandidateDriverExchangesListOffsetAndFetch(t *testing.T) {
	...
	observation, err := observeDifferentialKafkaProxyCandidate(differentialKafkaProxyCases()[0], port)
	if err != nil {
...
func TestDifferentialKafkaProxyOraclePacketParserPreservesPubSubRecord(t *testing.T) {
	...
	observation, err := parseDifferentialKafkaProxyOraclePacket(differentialKafkaProxyCases()[0], packet)
	if err != nil {
	...
	if _, err := parseDifferentialKafkaProxyOraclePacket(
		differentialKafkaProxyCases()[0], packet[:len(packet)-1],
