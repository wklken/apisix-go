...
func TestC6DifferentialCasesDeclareOnlyRequiredExtraSemanticHeaders(t *testing.T) {
	tests := []struct {
		...
		{name: "azure-functions", cases: differentialAzureFunctionsCases(), want: []string{"X-Functions-Key"}},
		{name: "opa", cases: differentialOPACases(), want: []string{"Content-Type"}},
		{name: "dingtalk-auth", cases: differentialDingTalkAuthCases(), want: []string{"Content-Type", "X-Userinfo"}},
		{name: "data-mask", cases: differentialDataMaskCases(), want: []string{"Content-Type"}},
		{name: "error-log-logger", cases: differentialErrorLogLoggerCases(), want: []string{
			"Content-Type", "X-ClickHouse-Database", "X-ClickHouse-Key", "X-ClickHouse-User", "X-Consumer-Username",
		}},
		{name: "feishu-auth", cases: differentialFeishuAuthCases(), want: []string{"Content-Type", "X-Userinfo"}},
		{name: "clickhouse-logger", cases: differentialClickHouseLoggerCases(), want: []string{
			"Content-Type", "X-ClickHouse-Database", "X-ClickHouse-Key", "X-ClickHouse-User",
		}},
		{name: "elasticsearch-logger", cases: differentialElasticsearchLoggerCases(), want: []string{"Content-Type"}},
		{name: "http-logger", cases: differentialHTTPLoggerCases(), want: []string{"Content-Type"}},
		{name: "lago", cases: differentialLagoCases(), want: []string{"Authorization", "Content-Type"}},
		{name: "loggly", cases: differentialLogglyCases(), want: []string{"Content-Type", "X-LOGGLY-TAG"}},
		{name: "loki-logger", cases: differentialLokiLoggerCases(), want: []string{"Content-Type", "X-Scope-OrgID"}},
		{name: "opentelemetry", cases: differentialOpenTelemetryCases(), want: []string{
			"Content-Encoding", "Content-Type", "X-Differential-OTel", "X-Request-Id", "X-Tenant",
		}},
		{name: "splunk-hec-logging", cases: differentialSplunkHECLoggingCases(), want: []string{"Content-Type"}},
		{name: "tencent-cloud-cls", cases: differentialTencentCloudCLSCases(), want: []string{"Content-Type"}},
		{name: "zipkin", cases: differentialZipkinCases(), want: []string{
