...
func DefaultClientOptions() ClientOptions {
	remotingConfig := remote.DefaultRemotingClientConfig
	opts := ClientOptions{
		InstanceName:         "DEFAULT",
		RetryTimes:           3,
		ClientIP:             utils.LocalIP,
		RemotingClientConfig: &remotingConfig,
	}
	return opts
}
...
func GetOrNewRocketMQClient(option ClientOptions, callbackCh chan interface{}) RMQClient {
	...
	if loaded {
		...
		client.remoteClient.RegisterRequestFunc(
			ReqNotifyConsumerIdsChanged,
		...
		client.remoteClient.RegisterRequestFunc(
			...
			func(req *remote.RemotingCommand, addr net.Addr) *remote.RemotingCommand {
				...
				if !exist {
					...
					if ok {
						consumeMessageDirectlyResult = cli.consumeMessageDirectly(
							msg,
							header.consumerGroup,
							header.brokerName,
						)
					}
...
func (c *rmqClient) Start() {
	...
	c.once.Do(func() {
		...
		c.startTask(func(ctx context.Context) {
			...
			if !waitTaskDelay(ctx, 10*time.Second) {
				return
			}
			op()
		...
		c.startTask(func(ctx context.Context) {
			...
			op := func() {
				...
				c.UpdateTopicRouteInfoContext(ctx)
			}
			if !waitTaskDelay(ctx, 10*time.Millisecond) {
				return
			}
			op()
		...
		c.startTask(func(ctx context.Context) {
			op := func() {
				...
				c.SendHeartbeatToAllBrokerWithContext(ctx)
			}

			if !waitTaskDelay(ctx, time.Second) {
				return
			}
			op()
...
func (c *rmqClient) startTask(fn func(context.Context)) {
	c.taskWG.Add(1)
...
func (c *rmqClient) InvokeAsync(ctx context.Context, addr string, request *remote.RemotingCommand,
	f func(*remote.RemotingCommand, error),
) error {
	if c.close {
...
func (c *rmqClient) SendHeartbeatToAllBrokerWithLock() {
	c.SendHeartbeatToAllBrokerWithContext(context.Background())
}

func (c *rmqClient) SendHeartbeatToAllBrokerWithContext(ctx context.Context) {
	c.hbMutex.Lock()
	...
	c.GetNameSrv().(*namesrvs).brokerAddressesMap.Range(func(key, value interface{}) bool {
		...
		for id, addr := range data.BrokerAddresses {
			...
			response, err := c.remoteClient.InvokeSync(requestCtx, addr, cmd)
			if err != nil {
				cancel()
...
func (c *rmqClient) UpdateTopicRouteInfo() {
	c.UpdateTopicRouteInfoContext(context.Background())
}

func (c *rmqClient) UpdateTopicRouteInfoContext(ctx context.Context) {
	...
	for topic := range allTopics {
		...
		data, changed, _ := c.GetNameSrv().UpdateTopicRouteInfoContext(ctx, topic)
		if ctx.Err() != nil {
			return
		}
...
func (c *rmqClient) ProcessSendResponse(
	brokerName string,
	cmd *remote.RemotingCommand,
	resp *primitive.SendResult,
	msgs ...*primitive.Message,
...
func (c *rmqClient) consumeMessageDirectly(
