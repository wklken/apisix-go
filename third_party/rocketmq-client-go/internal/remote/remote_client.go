...
func (c *remotingClient) InvokeSync(
	ctx context.Context,
	addr string,
...
func (c *remotingClient) InvokeAsync(
	ctx context.Context,
	addr string,
	request *RemotingCommand,
...
func (c *remotingClient) connect(ctx context.Context, addr string) (*tcpConnWrapper, error) {
	...
	if err := lockContext(ctx, &c.connectionLocker); err != nil {
		return nil, err
	}
...
func (c *remotingClient) doRequest(ctx context.Context, conn *tcpConnWrapper, request *RemotingCommand) error {
	if err := lockContext(ctx, &conn.Mutex); err != nil {
		return err
	}
	defer conn.Unlock()
