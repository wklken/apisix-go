...
func (s *namesrvs) UpdateTopicRouteInfo(topic string) (*TopicRouteData, bool, error) {
	return s.UpdateTopicRouteInfoContext(context.Background(), topic)
}

func (s *namesrvs) UpdateTopicRouteInfoContext(ctx context.Context, topic string) (*TopicRouteData, bool, error) {
	return s.UpdateTopicRouteInfoWithDefaultContext(ctx, topic, "", 0)
}

func (s *namesrvs) CheckTopicRouteHasTopic(topic string) bool {
	_, err := s.queryTopicRouteInfoFromServer(context.Background(), topic)
	if err != nil {
		return false
	}
	return true
...
func (s *namesrvs) UpdateTopicRouteInfoWithDefault(
	topic string,
	defaultTopic string,
	defaultQueueNum int,
) (*TopicRouteData, bool, error) {
	return s.UpdateTopicRouteInfoWithDefaultContext(context.Background(), topic, defaultTopic, defaultQueueNum)
}

func (s *namesrvs) UpdateTopicRouteInfoWithDefaultContext(
	ctx context.Context,
	topic string,
	defaultTopic string,
	defaultQueueNum int,
) (*TopicRouteData, bool, error) {
	if err := lockNamesrvContext(ctx, &s.lockNamesrv); err != nil {
...
func lockNamesrvContext(ctx context.Context, lock *sync.Mutex) error {
	for !lock.TryLock() {
...
func (s *namesrvs) FindBrokerAddressInSubscribe(
	brokerName string,
	brokerId int64,
	onlyThisBroker bool,
) *FindBrokerResult {
...
func (s *namesrvs) FetchSubscribeMessageQueues(topic string) ([]*primitive.MessageQueue, error) {
	routeData, err := s.queryTopicRouteInfoFromServer(context.Background(), topic)
	if err != nil {
		return nil, err
	}
...
func (s *namesrvs) queryTopicRouteInfoFromServer(ctx context.Context, topic string) (*TopicRouteData, error) {
	...
	for i := 0; i < s.Size(); i++ {
		...
		response, err = s.nameSrvClient.InvokeSync(attemptCtx, s.getNameServerAddress(), rc)
...
func (s *namesrvs) topicRouteDataIsChange(oldData *TopicRouteData, newData *TopicRouteData) bool {
	...
	sort.Slice(oldDataCloned.BrokerDataList, func(i, j int) bool {
		return strings.Compare(
			oldDataCloned.BrokerDataList[i].BrokerName,
			oldDataCloned.BrokerDataList[j].BrokerName,
		) > 0
	})
	...
	sort.Slice(newDataCloned.BrokerDataList, func(i, j int) bool {
		return strings.Compare(
			newDataCloned.BrokerDataList[i].BrokerName,
			newDataCloned.BrokerDataList[j].BrokerName,
		) > 0
	})
