...
// WithTlsVerify controls certificate and server-name verification when TLS is
// enabled. The default remains false for APISIX compatibility.
func WithTlsVerify(verify bool) Option {
	return func(opts *producerOptions) {
		opts.ClientOptions.RemotingClientConfig.TLSVerify = verify
	}
}

// WithTlsRootCAs injects a trust pool for callers that own the trust policy,
// such as tests. It is not exposed through the APISIX plugin schema.
func WithTlsRootCAs(roots *x509.CertPool) Option {
	return func(opts *producerOptions) {
		opts.ClientOptions.RemotingClientConfig.TLSRootCAs = roots
	}
}
