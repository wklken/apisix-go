...
func (p *defaultProducer) Request(
	ctx context.Context,
...
func (p *defaultProducer) RequestAsync(
	ctx context.Context,
	timeout time.Duration,
	callback internal.RequestCallback,
	msg *primitive.Message,
) error {
	if err := p.checkMsg(msg); err != nil {
		return err
...
func (p *defaultProducer) SendAsync(
	ctx context.Context,
...
func (p *defaultProducer) sendAsync(
	ctx context.Context,
	msg *primitive.Message,
	h func(context.Context, *primitive.SendResult, error),
) error {
...
func (p *defaultProducer) tryToFindTopicPublishInfo(topic string) *internal.TopicPublishInfo {
	return p.tryToFindTopicPublishInfoContext(context.Background(), topic)
}

func (p *defaultProducer) tryToFindTopicPublishInfoContext(
	ctx context.Context,
	topic string,
) *internal.TopicPublishInfo {
	v, exist := p.publishInfo.Load(topic)
	if !exist {
		data, changed, err := p.client.GetNameSrv().UpdateTopicRouteInfoContext(ctx, topic)
	...
	if !exist {
		data, changed, _ := p.client.GetNameSrv().
			UpdateTopicRouteInfoWithDefaultContext(ctx, topic, p.options.CreateTopicKey, p.options.DefaultTopicQueueNums)
		p.client.UpdatePublishInfo(topic, data, changed)
		v, exist = p.publishInfo.Load(topic)
	}
...
func (p *defaultProducer) selectMessageQueue(
	ctx context.Context,
	msg *primitive.Message,
	lastBrokerName string,
) *primitive.MessageQueue {
	result := p.tryToFindTopicPublishInfoContext(ctx, msg.Topic)
...
func (tp *transactionProducer) endTransaction(
